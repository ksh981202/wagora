...
function resolveActiveFullPartsTab(rawTab: string | null): (typeof FULL_PARTS_TABS)[number] {
  ...
  return FULL_PARTS_TABS.find((tab) => tab === trimmed || extractPureThemeKeyword(tab) === pure) ?? '전체';
}
...
export default function FullPartsListPage() {
  ...
  const activeTab = useMemo(() => resolveActiveFullPartsTab(searchParams.get('tab')), [searchParams]);
  const activeTabKeyword = fullPartsTabKeywordForQuery(activeTab);
  ...
  const setActiveTab = useCallback(
    (tab: (typeof FULL_PARTS_TABS)[number]) => {
  ...
  const saveListScrollPosition = useCallback(() => {
    try {
      sessionStorage.setItem(FULL_PARTS_SCROLL_Y_KEY, window.scrollY.toString());
      sessionStorage.setItem(FULL_PARTS_SCROLL_ITEMS_KEY, galleryItems.length.toString());
    } catch {
  ...
  useEffect(() => {
    ...
    const savedY = sessionStorage.getItem(FULL_PARTS_SCROLL_Y_KEY);
    if (!savedY) return;

    const savedItemsRaw = sessionStorage.getItem(FULL_PARTS_SCROLL_ITEMS_KEY);
    ...
    const timer = window.setTimeout(() => {
      window.scrollTo(0, y);
      sessionStorage.removeItem(FULL_PARTS_SCROLL_Y_KEY);
      sessionStorage.removeItem(FULL_PARTS_SCROLL_ITEMS_KEY);
    }, 100);
      ...
      <main className="grid grid-cols-2 gap-4 px-5 pb-8 pt-4">
        {isLoading ? (
          Array.from({ length: 8 }, (_, index) => (
            <article key={`full-parts-skel-${index}`} className="flex flex-col gap-2" aria-hidden>
          ...
          <>
            ...
            {isFetchingNextPage
              ? [0, 1].map((index) => (
                  <article key={`full-parts-next-skel-${index}`} className="flex flex-col gap-2" aria-hidden>
                    <div className="w-full aspect-[3/4] rounded-xl overflow-hidden bg-gray-100 animate-pulse" />
                    <div className="mx-auto h-4 w-3/4 animate-pulse rounded bg-gray-100" />
                  </article>
                ))
