...
const PARTS_TAB_LABEL_EN: Record<(typeof PARTS_TAB_LABELS)[number], string> = {
  전체: 'All',
  '💎 스톤/큐빅': '💎 Stone/Cubic',
  '🎀 리본': '🎀 Ribbon',
  '⚪ 진주': '⚪ Pearl',
  '⛓️ 메탈/체인': '⛓️ Metal/Chain',
  '🦋 나비': '🦋 Butterfly',
};
...
function displayPartsTabLabel(tab: (typeof PARTS_TAB_LABELS)[number], isEnglish: boolean): string {
  return isEnglish ? PARTS_TAB_LABEL_EN[tab] : tab;
}
        ...
        <section className="min-w-0 w-full flex flex-nowrap gap-2 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] px-4 pb-2 pt-1 whitespace-nowrap scroll-smooth [-webkit-overflow-scrolling:touch]">
          {PARTS_TAB_LABELS.map((tab) => {
            ...
            return (
              ...
              >
                {displayPartsTabLabel(tab, isEnglish)}
              </button>
            );
          })}
