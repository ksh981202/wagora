...
function resolveActivePopularMoodTab(rawTab: string | null): PopularMoodTabLabel {
  ...
  return POPULAR_MOOD_TABS.find((tab) => tab.label === trimmed || extractPureThemeKeyword(tab.label) === pure)?.label ?? '전체';
}
...
export default function PopularMoodListPage() {
  ...
  const activeTab = useMemo(() => resolveActivePopularMoodTab(searchParams.get('tab')), [searchParams]);
  const activeTabKeyword = popularMoodTabKeywordForQuery(activeTab);
  ...
  const saveListScrollPosition = useCallback(() => {
    try {
      sessionStorage.setItem(POPULAR_MOOD_LIST_SCROLL_Y_KEY, window.scrollY.toString());
      sessionStorage.setItem(POPULAR_MOOD_LIST_SCROLL_ITEMS_KEY, galleryItems.length.toString());
    } catch {
  ...
  useEffect(() => {
    ...
    const savedY = sessionStorage.getItem(POPULAR_MOOD_LIST_SCROLL_Y_KEY);
    if (!savedY) return;

    const savedItemsRaw = sessionStorage.getItem(POPULAR_MOOD_LIST_SCROLL_ITEMS_KEY);
    ...
    const timer = window.setTimeout(() => {
      window.scrollTo(0, y);
      sessionStorage.removeItem(POPULAR_MOOD_LIST_SCROLL_Y_KEY);
      sessionStorage.removeItem(POPULAR_MOOD_LIST_SCROLL_ITEMS_KEY);
    }, 100);
      ...
      <main className="grid grid-cols-2 gap-4 px-5 pb-8 pt-4">
        {isLoading ? (
          Array.from({ length: 8 }, (_, index) => (
            <article key={`popular-mood-list-skel-${index}`} className="flex flex-col gap-2" aria-hidden>
          ...
          <>
            ...
            {isFetchingNextPage
              ? [0, 1].map((index) => (
                  <article key={`popular-mood-list-next-skel-${index}`} className="flex flex-col gap-2" aria-hidden>
                    <div className="w-full aspect-[3/4] rounded-xl overflow-hidden bg-gray-100 animate-pulse" />
                    <div className="mx-auto h-4 w-3/4 animate-pulse rounded bg-gray-100" />
                  </article>
                ))
