...
const PATTERN_TAB_LABEL_EN: Record<(typeof PATTERN_TAB_LABELS)[number], string> = {
  전체: 'All',
  '🌙 프렌치': '🌙 French',
  '🖌️ 마블': '🖌️ Marble',
  '🏁 체크': '🏁 Check',
...
function resolveActivePatternTab(rawTab: string | null): (typeof PATTERN_TAB_LABELS)[number] {
  const trimmed = rawTab?.trim();
  if (!trimmed || trimmed === DEFAULT_GALLERY_TAB) return '전체';
  const pure = extractPureThemeKeyword(trimmed);
  return PATTERN_TAB_LABELS.find((tab) => tab === trimmed || extractPureThemeKeyword(tab) === pure) ?? '전체';
}
...
function displayPatternTabLabel(tab: (typeof PATTERN_TAB_LABELS)[number], isEnglish: boolean): string {
  return isEnglish ? PATTERN_TAB_LABEL_EN[tab] : tab;
}
...
export default function PatternListPage() {
  ...
  const activeTabKeyword = patternTabKeywordForQuery(activeTab);
  ...
  const saveListScrollPosition = useCallback(() => {
    try {
      sessionStorage.setItem(PATTERN_LIST_SCROLL_Y_KEY, window.scrollY.toString());
      sessionStorage.setItem(PATTERN_LIST_SCROLL_ITEMS_KEY, galleryItems.length.toString());
    } catch {
  ...
  useEffect(() => {
    ...
    const savedY = sessionStorage.getItem(PATTERN_LIST_SCROLL_Y_KEY);
    if (!savedY) return;

    const savedItemsRaw = sessionStorage.getItem(PATTERN_LIST_SCROLL_ITEMS_KEY);
    ...
    const timer = window.setTimeout(() => {
      window.scrollTo(0, y);
      sessionStorage.removeItem(PATTERN_LIST_SCROLL_Y_KEY);
      sessionStorage.removeItem(PATTERN_LIST_SCROLL_ITEMS_KEY);
    }, 100);
        ...
        <section className="min-w-0 w-full flex flex-nowrap gap-2 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] px-4 pb-2 pt-1 whitespace-nowrap scroll-smooth [-webkit-overflow-scrolling:touch]">
          {PATTERN_TAB_LABELS.map((tab) => {
            ...
            return (
              ...
              >
                {displayPatternTabLabel(tab, isEnglish)}
              </button>
            );
          })}
      ...
      <main className="grid grid-cols-2 gap-4 px-5 pb-8 pt-4">
        {isLoading ? (
          Array.from({ length: 8 }, (_, index) => (
            <article key={`pattern-list-skel-${index}`} className="flex flex-col gap-2" aria-hidden>
          ...
          <>
            ...
            {isFetchingNextPage
              ? [0, 1].map((index) => (
                  <article key={`pattern-list-next-skel-${index}`} className="flex flex-col gap-2" aria-hidden>
                    <div className="w-full aspect-[3/4] rounded-xl overflow-hidden bg-gray-100 animate-pulse" />
                    <div className="mt-2 flex w-full flex-col gap-1 px-1">
                      <div className="h-3.5 w-full rounded bg-gray-200 animate-pulse" />
                      <div className="h-3.5 w-2/3 rounded bg-gray-200 animate-pulse" />
