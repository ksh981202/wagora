...
function buildPopularArtOrFilter(): string {
  const parts: string[] = [];
...
function usePopularArtQuery(maxLimit: number) {
  return useQuery({
    queryKey: ["nail-designs", "popular-art", { maxLimit }],
    staleTime: 5 * 60 * 1000,
...
export default function PopularArtListPage() {
  ...
  const saveListScrollPosition = useCallback(() => {
    try {
      sessionStorage.setItem(POPULAR_ART_SCROLL_Y_KEY, window.scrollY.toString());
    } catch {
  ...
  useEffect(() => {
    ...
    const savedY = sessionStorage.getItem(POPULAR_ART_SCROLL_Y_KEY);
    if (!savedY) return;
    const y = Number.parseInt(savedY, 10);
    if (Number.isNaN(y)) return;
    const timer = window.setTimeout(() => {
      window.scrollTo(0, y);
      sessionStorage.removeItem(POPULAR_ART_SCROLL_Y_KEY);
    }, 100);
