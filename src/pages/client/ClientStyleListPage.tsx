...
export default function ClientStyleListPage() {
  ...
  const activeTabLabel = useMemo(
    () => resolveActiveStyleTabLabel(searchParams.get('tab')),
    [searchParams],
  )
  const activeTabKeyword = styleTabKeywordForQuery(activeTabLabel)
            ...
            <ul className="grid grid-cols-2 gap-4 px-5 pb-8 pt-4">
              {isLoading ? (
                Array.from({ length: 8 }, (_, i) => (
                  <li key={`style-list-skel-${i}`} aria-hidden>
                ...
                <>
                  ...
                  {isFetchingNextPage
                    ? [0, 1].map((i) => (
                        <li key={`style-list-next-skel-${i}`} aria-hidden>
                          <div className="flex flex-col gap-2">
                            <div className="aspect-[3/4] w-full min-h-0 animate-pulse rounded-xl bg-gray-100" />
                            <div className="mx-auto mt-2 h-4 w-3/4 animate-pulse rounded bg-gray-100" />
                          </div>
